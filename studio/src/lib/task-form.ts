...
export function cloneTaskFormDefaults(task: Task): TaskFormDefaults {
  ...
  return {
    ...
    prompt: task.prompt,
    quantity: 1,
    image_ratio: task.type === 'montage' ? 'auto' : (task.image_ratio || 'auto') as TaskFormDefaults['image_ratio'],
    image_capability_key: task.type === 'montage' ? '' : task.image_capability_key ?? '',
