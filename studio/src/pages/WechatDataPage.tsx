...
export default function WechatDataPage() {
  ...
  return <div className="space-y-5">
    ...
    <Dialog open={bindOpen} onOpenChange={(open) => !bindMutation.isPending && setBindOpen(open)}><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle>确认已人工发布</DialogTitle></DialogHeader><div className="space-y-3"><p className="text-sm text-muted-foreground">请确认草稿已在公众号后台正式发布。文章 URL 可选，用于打开文章和优先匹配导入数据，不会触发自动发布。</p><Input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="可选：https://mp.weixin.qq.com/s/..." aria-label="文章 URL（可选）" /><ContentSourceLink url={url} /><p className="text-xs text-muted-foreground">导入数据匹配成功后会自动补齐链接，无需重复填写。</p>{bindMutation.error && <p className="text-sm text-destructive">{(bindMutation.error as Error).message}</p>}</div><DialogFooter><Button variant="outline" onClick={() => setBindOpen(false)}>取消</Button><Button disabled={bindMutation.isPending} onClick={() => bindMutation.mutate()}>{bindMutation.isPending ? <Loader2 className="animate-spin" /> : <Link2 />}确认已发布</Button></DialogFooter></DialogContent></Dialog>
  </div>
}
