...
function ImportPreview({ preview }: { preview: WechatAnalyticsImportPreview }) {
  return <div className="space-y-4">
    <div className="grid grid-cols-2 gap-3 rounded-md border bg-muted/20 p-3 text-sm sm:grid-cols-4">
      ...
      <div><p className="text-xs text-muted-foreground">来源</p><p className="mt-1 truncate font-medium">{preview.source}</p></div>
      <div><p className="text-xs text-muted-foreground">解析器</p><p className="mt-1 font-medium">{preview.parser_version}</p></div>
    </div>
    <div>
    ...
    <div>
      <p className="mb-2 text-sm font-medium">前 {Math.min(10, preview.rows.length)} 行预览</p>
      <div className="max-h-64 overflow-auto rounded-md border">
        <Table>
          <TableHeader><TableRow><TableHead>标题</TableHead><TableHead>日期</TableHead><TableHead>阅读</TableHead><TableHead>分享</TableHead><TableHead>完成率</TableHead></TableRow></TableHeader>
          <TableBody>{preview.rows.map((row) => <TableRow key={row.source_row}><TableCell className="min-w-64 max-w-80 truncate">{row.title}</TableCell><TableCell className="whitespace-nowrap">{row.published_date ? formatDate(row.published_date) : '—'}</TableCell><TableCell>{formatNumber(row.read_users)}</TableCell><TableCell>{formatNumber(row.share_users)}</TableCell><TableCell>{formatRate(row.read_completion_rate)}</TableCell></TableRow>)}</TableBody>
        </Table>
      </div>
    </div>
  </div>
...
export default function WechatDataPage() {
  ...
  return <div className="space-y-5">
    <PageHeader title="公众号内容" description="文章、发布与数据在同一条内容流中管理。未认证账号会自动转为人工发布。">
    ...
    {activeProjectId && !articlesQuery.isLoading && <section className="overflow-hidden rounded-md border bg-card">
      ...
      <div className="overflow-x-auto max-md:overflow-visible"><Table className="max-md:block"><TableHeader className="max-md:hidden"><TableRow><TableHead>文章</TableHead><TableHead>发布渠道</TableHead><TableHead>当前状态</TableHead><TableHead>数据来源</TableHead><TableHead>阅读</TableHead><TableHead>分享</TableHead><TableHead>阅读完成率</TableHead><TableHead className="text-right">下一步</TableHead></TableRow></TableHeader><TableBody className="max-md:block">{visibleArticles.map((article) => {
        ...
        return <TableRow key={publication.id} className="max-md:grid max-md:grid-cols-6 max-md:gap-x-3 max-md:gap-y-3 max-md:p-3">
          <TableCell className="min-w-72 max-w-[380px] max-md:col-span-6 max-md:min-w-0 max-md:max-w-none max-md:p-0"><button type="button" className="max-w-full truncate text-left font-medium hover:text-primary" onClick={() => setSelectedArticle(article)}>{publication.draft_title || '未命名文章'}</button><p className="mt-1 text-xs text-muted-foreground">{formatDate(publication.published_at)}{publication.article_url && <a className="ml-2 inline-flex items-center gap-1 text-primary" href={publication.article_url} target="_blank" rel="noreferrer">打开 <ExternalLink className="h-3 w-3" /></a>}</p></TableCell>
    ...
    <Sheet open={Boolean(selectedArticle) && !bindOpen} onOpenChange={(open) => { if (!open) setSelectedArticle(null) }}><SheetContent side="right" className="w-full overflow-y-auto sm:max-w-xl"><SheetHeader className="border-b pr-14"><SheetTitle>{selectedArticle?.publication.draft_title || '文章详情'}</SheetTitle><SheetDescription>发布与数据时间线</SheetDescription></SheetHeader>{selectedArticle && <div className="space-y-5 px-5 pb-6">
      ...
      {needsManualBind(selectedArticle) && <Button className="w-full" onClick={() => openBind(selectedArticle)}><Link2 />已在公众号后台发布</Button>}
    </div>}</SheetContent></Sheet>

    <Dialog open={importOpen} onOpenChange={(open) => { if (!open && !previewMutation.isPending && !importMutation.isPending) closeImport() }}><DialogContent className="max-h-[88vh] overflow-y-auto sm:max-w-4xl"><DialogHeader><DialogTitle>{preview ? '确认导入数据' : '选择公众号数据文件'}</DialogTitle></DialogHeader><div className="space-y-4">{!preview && <><label htmlFor="wechat-import-file" className="flex min-h-32 cursor-pointer flex-col items-center justify-center rounded-md border border-dashed bg-muted/20 px-5 text-center hover:border-primary/50"><FileSpreadsheet className="mb-3 h-8 w-8 text-primary" /><span className="text-sm font-medium">选择公众号导出文件</span><span className="mt-1 text-xs text-muted-foreground">支持 .xls 和 .xlsx，最大 20 MB</span><input id="wechat-import-file" className="sr-only" type="file" accept=".xls,.xlsx,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={(event) => { stageFile(event.target.files?.[0]); event.currentTarget.value = '' }} /></label>{stagedFile && <div className="flex items-center justify-between rounded-md border p-3 text-sm"><span className="truncate">{stagedFile.name}</span><Button size="icon-sm" variant="ghost" aria-label="移除文件" onClick={() => setStagedFile(null)}><X /></Button></div>}</>}{preview && <ImportPreview preview={preview} />}{fileError && <p className="text-sm text-destructive">{fileError}</p>}{previewMutation.error && <p className="text-sm text-destructive">{(previewMutation.error as Error).message}</p>}{importMutation.error && <p className="text-sm text-destructive">{(importMutation.error as Error).message}</p>}</div><DialogFooter>{preview ? <Button variant="outline" onClick={() => { setPreview(null); setUploadId('') }}>重新选择</Button> : <Button variant="outline" onClick={closeImport}>取消</Button>}{preview ? <Button disabled={importMutation.isPending} onClick={() => importMutation.mutate()}>{importMutation.isPending ? <><Loader2 className="animate-spin" />正在导入</> : '确认导入'}</Button> : <Button disabled={!stagedFile || previewMutation.isPending} onClick={() => stagedFile && previewMutation.mutate(stagedFile)}>{previewMutation.isPending ? <><Loader2 className="animate-spin" />正在解析</> : '预览数据'}</Button>}</DialogFooter></DialogContent></Dialog>

    <Dialog open={bindOpen} onOpenChange={(open) => !bindMutation.isPending && setBindOpen(open)}><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle>确认已人工发布</DialogTitle></DialogHeader><div className="space-y-3"><p className="text-sm text-muted-foreground">请确认草稿已在公众号后台正式发布。文章 URL 可选，用于打开文章和优先匹配导入数据，不会触发自动发布。</p><Input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="可选：https://mp.weixin.qq.com/s/..." aria-label="文章 URL（可选）" />{bindMutation.error && <p className="text-sm text-destructive">{(bindMutation.error as Error).message}</p>}</div><DialogFooter><Button variant="outline" onClick={() => setBindOpen(false)}>取消</Button><Button disabled={bindMutation.isPending} onClick={() => bindMutation.mutate()}>{bindMutation.isPending ? <Loader2 className="animate-spin" /> : <Link2 />}确认已发布</Button></DialogFooter></DialogContent></Dialog>
  </div>
}
