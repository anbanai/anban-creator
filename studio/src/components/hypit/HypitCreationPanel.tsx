...
function assets(values: HypitAttachment[]): HypitAsset[] {
  return values.filter(v => ['image', 'video', 'audio'].includes(v.type)).map(v => ({ ...v.hypitAsset, type: v.type as HypitAsset['type'], url: v.url, file_name: v.file_name, mime_type: v.content_type, file_size: v.size }))
}
export function HypitCreationPanel({ form, defaults = false, remix = false, sourceTaskId, briefField, onReadyChange, onUploadingChange }: { form: UseFormReturn<any>; defaults?: boolean; remix?: boolean; sourceTaskId?: string; briefField?: ReactNode; onReadyChange?: (ready: boolean) => void; onUploadingChange?: (uploading: boolean) => void }) {
  ...
  return <section className="space-y-4" aria-label={defaults ? '视频复刻默认设置' : '视频复刻设置'}>
    ...
    {!defaults && <>{briefField ?? <FormField control={form.control} name={`${root}.brief`} render={({ field }) => <FormItem><FormLabel>复刻要求</FormLabel><Textarea aria-label={defaults ? "素材使用说明" : "复刻要求"} {...field} value={field.value ?? ''} placeholder="描述希望保留的镜头、节奏及需要替换的内容" /><FormMessage /></FormItem>} />}
      <FormField control={form.control} name={`${root}.reference`} render={({ field }) => <FormItem><FormLabel>{remix ? '参考视频（可选，沿用原工程）' : '参考视频'}</FormLabel><Input aria-label="参考视频链接" placeholder="https://… 参考视频链接" value={field.value?.type === 'video_url' ? field.value.url ?? '' : ''} onChange={e => field.onChange(e.target.value ? { type: 'video_url', url: e.target.value } : undefined)} />
        <ReferenceMaterialInput value={field.value && field.value.type !== 'video_url' ? attachments([field.value]) : []} onChange={v => field.onChange(assets(v)[0])} allowedTypes={['video']} maxCount={1} maxFileBytes={limits?.max_asset_bytes} uploadPurpose="hypit_asset" onUploadingChange={setReferenceUploading} onFailuresChange={setReferenceFailed} hint="上传一段参考视频，或填写上方链接。" /><FormMessage /></FormItem>} />
      <FormField control={form.control} name={`${root}.source_assets`} render={({ field }) => <FormItem><FormLabel>补充素材（可选）</FormLabel><ReferenceMaterialInput value={attachments(field.value ?? [])} onChange={v => field.onChange(assets(v))} allowedTypes={['image', 'video', 'audio']} maxCount={limits?.max_assets ?? 0} maxFileBytes={limits?.max_asset_bytes} uploadPurpose="hypit_asset" onUploadingChange={setAssetsUploading} onFailuresChange={setAssetsFailed} hint="添加要替换的人物、产品、镜头或音频。" /></FormItem>} /></>}
    <div className="grid gap-3 sm:grid-cols-3">
      <FormField control={form.control} name={`${root}.preferences.duration_seconds`} render={({ field }) => <FormItem><FormLabel>目标时长（秒）</FormLabel><Input aria-label="目标时长（秒）" type="number" min={0} max={limits?.max_duration_seconds} value={field.value ?? ''} onChange={e => field.onChange(e.target.value ? Number(e.target.value) : 0)} placeholder="跟随参考视频" /><FormMessage /></FormItem>} />
    ...
    {defaults && <FormField control={form.control} name={`${root}.asset_guidance`} render={({ field }) => <FormItem><FormLabel>素材使用说明</FormLabel><Textarea aria-label={defaults ? "素材使用说明" : "复刻要求"} {...field} value={field.value ?? ''} /></FormItem>} />}
