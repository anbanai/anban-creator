...
describe('video replication task integration', () => {
  ...
  it('keeps a valid same-type project switch ready and retains the reference', async () => {
    ...
    await waitFor(() => expect(screen.getByLabelText('目标时长（可选）')).toHaveValue(40))
    expect(screen.getByLabelText('主参考视频链接')).toHaveValue('https://example.com/ref.mp4')
  ...
  it('creates from video link and inherited defaults without image capability controls', async () => {
    ...
    expect(screen.getByLabelText('复刻要求').closest('[data-slot="agent-prompt-input"]')).not.toBeNull()
  ...
  it('uses frozen source capabilities when global duration limits have been reduced', async () => {
    ...
    renderDialog({ mode: 'clone', sourceTask: source, initialProjectId: undefined })
    await screen.findByLabelText('目标时长（可选）')
    ...
    expect(screen.getByLabelText('目标时长（可选）')).toHaveValue(180)
    fireEvent.change(screen.getByLabelText('目标时长（可选）'), { target: { value: '' } })
  ...
  it('remixes a saved follow-source duration without a hidden validation error', async () => {
    const source = { ...fixtures.sourceTask, type: 'hypit', project_id: hypitProject.id, hypit_input: { brief: 'old', preferences: { duration_seconds: 0 } } } as Task
    renderDialog({ mode: 'clone', sourceTask: source, initialProjectId: undefined })
    expect(await screen.findByLabelText('目标时长（可选）')).toHaveValue(null)
    const submit = screen.getByRole('button', { name: '再次改编' })
    await waitFor(() => expect(submit).toBeEnabled())
    expect(screen.queryByText('时长必须为正整数秒')).not.toBeInTheDocument()
