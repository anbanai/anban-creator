...
通过 Claude Code / Codex 等客户端调用平台能力时，平台模型按实际使用的模型定价。当前 MCP 直接保留媒体、理解、转换、评分等工具；文章写作、选题研究、SEO 优化、大纲生成等生成式写作流程迁移到 Skills，不再作为 MCP 写作生成工具暴露。
