# Rednote Profile Analysis Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Xiaohongshu account setup accepts share text, fetches profile and top visible posts, then AI-enriches positioning, keywords, and visual style while hiding account homepage for other platforms.
...
- `server/platform/provider.go`: extend `PlatformProfile` with `Keywords` and `Style`, and add a `RednotePost` DTO.
- `server/platform/rednote.go`: extract Xiaohongshu URLs from arbitrary text, parse visible post metadata, normalize counts, rank top posts.
- `server/platform/rednote_test.go`: TDD coverage for URL extraction, post ranking, and profile raw data.
- `server/handler/channel.go`: accept share text for rednote, block auto-fetch for article/xls, optionally enrich rednote profile through AI.
...
- `server/model/platform.go`: remove homepage fields and auto-fetch for article/xls; keep rednote homepage with share-text wording.
- `studio/src/types/channel.ts`: expose `keywords` and `style` in `PlatformProfile`.
- `studio/src/pages/ChannelsPage.tsx`: hide profile input for non-rednote, enable fetch based on embedded supported URL, auto-fill keywords/style.

## Tasks

### Task 1: Rednote URL Extraction And Post Ranking

**Files:**
- Modify: `server/platform/provider.go`
- Modify: `server/platform/rednote.go`
- Test: `server/platform/rednote_test.go`
...
- [ ] Run `go test ./server/platform -run 'TestRednote' -count=1` and confirm the new tests fail.
- [ ] Implement URL extraction, `RednotePost`, count parsing, and top-post selection with SSRF-safe allowed hosts.
- [ ] Run `go test ./server/platform -run 'TestRednote' -count=1` and confirm it passes.
...
- [ ] Update frontend URL detection to support Xiaohongshu URLs embedded in share text.
...
- [ ] Run `git diff -- server/platform/provider.go server/platform/rednote.go server/platform/rednote_test.go server/handler/channel.go server/handler/channel_test.go server/main.go server/model/platform.go studio/src/types/channel.ts studio/src/pages/ChannelsPage.tsx`.
