...
Use `session.ID` as the asset ID. This keeps destination identity deterministic across a copy/database split failure while the separate tables retain distinct lifetime semantics. The function must: load and validate owner/purpose/expiry; return the linked asset for an already-finalized session; acquire a one-minute finalization lease; HEAD staging; validate exact size and normalized media type; persist the source ETag as the copy precondition; call `PromoteObject(staging, final, sourceETag)` and capture its target ETag; HEAD final; persist the target ETag as the recovery fingerprint; and use `repo.WithTx` to create or verify the immutable asset and complete the token-matched session. Source and target ETags must not be assumed equal. Release the claim best-effort on pre-commit failure, then delete only the staging object best-effort after database success. Never call `Provider.Read`, `ReadObject`, or an application-host copy loop.

The OSS implementation must retain `X-Oss-Copy-Source-If-Match` and `X-Oss-Forbid-Overwrite` and return the target ETag from `CopyObjectResult`. Treat `storage.ErrObjectAlreadyExists` as a retry path only after final metadata exactly matches the proposed asset and the durable source copy precondition is present.
