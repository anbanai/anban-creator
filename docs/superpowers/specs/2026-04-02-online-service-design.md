# AnbanWriter Online Service — Design Spec
...
AnbanWriter's content generation capabilities (小红书, 公众号图文, 小绿书) are only accessible via CLI within Claude Code. This limits usage to developers with local setup. The goal is to make these capabilities available as a multi-user SaaS web service.
