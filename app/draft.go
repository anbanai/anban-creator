...
平台说明：
  ...
  - 小红书发布请使用 MCP 工具 publish_content() 或 abwriter rednote export 导出`,
	}
...
注意：
  - 需要在配置文件中设置微信公众号账号信息
  - 运行 'abwriter account init' 创建配置文件
...
示例:
  abwriter draft test output.html cover.jpg
  abwriter draft test output.html cover.jpg --title "测试文章" --digest "摘要"`,
...
func runPublish(title, content, author, digest, coverID, outputDir, images string) error {
	...
	responseSuccess(map[string]any{
		...
		"message":      "草稿JSON已保存",
		"next_command": "abwriter draft article " + draftFile,
	})
