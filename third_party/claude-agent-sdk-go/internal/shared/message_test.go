...
func TestResultMessageModelUsageJSON(t *testing.T) {
	raw := []byte(`{
		"type":"result",
		"subtype":"success",
	...
	if !reflect.DeepEqual(message.ModelUsage, want) {
		t.Fatalf("ModelUsage = %#v, want %#v", message.ModelUsage, want)
	}

	roundTripJSON, err := json.Marshal(&message)
	if err != nil {
		t.Fatalf("Failed to marshal ResultMessage: %v", err)
	}
	var roundTrip ResultMessage
	if err := json.Unmarshal(roundTripJSON, &roundTrip); err != nil {
		t.Fatalf("Failed to unmarshal round-trip ResultMessage: %v", err)
	}
	if !reflect.DeepEqual(roundTrip.ModelUsage, want) {
		t.Fatalf("round-trip ModelUsage = %#v, want %#v", roundTrip.ModelUsage, want)
	}
}

func TestResultMessageModelUsageMissingIsNil(t *testing.T) {
	var message ResultMessage
	if err := json.Unmarshal([]byte(`{"type":"result","subtype":"success"}`), &message); err != nil {
		t.Fatalf("Failed to unmarshal ResultMessage: %v", err)
	}
	if message.ModelUsage != nil {
		t.Fatalf("ModelUsage = %#v, want nil", message.ModelUsage)
	}
}
