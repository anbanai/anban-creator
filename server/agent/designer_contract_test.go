...
func TestDesignerAgentKeepsMCPAndSkillLoadingContract(t *testing.T) {
	...
	required := []string{
		...
		"`mcp__plugin_anban_creator__generate_image`",
		"`mcp__plugin_anban_creator__analyze_image`",
		"`mcp__plugin_anban_creator__download_image`",
		"`mcp__plugin_anban_creator__prepare_workspace`",
		"`mcp__plugin_anban_creator__update_task_progress`",
		"ANBAN_API_URL",
		"ANBAN_DEFAULT_PROJECT",
		"ANBAN_API_KEY",
		"无法看到 `mcp__plugin_anban_creator__generate_image`",
		"停止并报告 MCP 工具未注入",
		"不要绕过 MCP",
		"插件 skill 路径解析",
...
func TestClaudeCodePluginDocsKeepDesignerMCPContract(t *testing.T) {
	...
	required := []string{
		...
		"`mcp__plugin_anban_creator__...` tools",
