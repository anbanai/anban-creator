...
func TestClaudeAgentsOwnFinalFeedback(t *testing.T) {
	...
	for _, name := range agents {
		t.Run(name, func(t *testing.T) {
			body := readRepoFile(t, filepath.Join("../../harness/agents", name+".md"))
...
func TestClaudeAgentFeedbackFollowsDeliveryReport(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			body := readRepoFile(t, filepath.Join("../../harness/agents", tt.name+".md"))
...
func TestClaudeCodePluginManifestMatchesOfficialBestPracticeFields(t *testing.T) {
	...
	raw := readRepoFile(t, "../../harness/.claude-plugin/plugin.json")
	...
	raw = readRepoFile(t, "../../harness/.claude-plugin/marketplace.json")
...
func TestClaudeCodePluginAgentsUseOnlySupportedFrontmatterFields(t *testing.T) {
	...
	for _, entry := range entries {
		...
		for _, skill := range frontmatterStringList(fm["skills"]) {
			skillPath := filepath.Join(repoRoot(t), "harness", "skills", skill, "SKILL.md")
...
func TestClaudeCodeHooksUseExecFormForPluginPathCommands(t *testing.T) {
	...
	raw := readRepoFile(t, "../../harness/hooks/hooks.json")
...
func TestUnifiedSkillsLinkConcreteExamples(t *testing.T) {
	...
	for _, entry := range entries {
		...
		for _, boilerplate := range []string{
			...
			if strings.Contains(examplesBody, boilerplate) {
				t.Fatalf("%s contains context-only template prose %q", examplesPath, boilerplate)
			}
		}

		for _, mirror := range []string{"harness"} {
