...
// budget. Long-text tool handlers can block on rendering or service calls for
// minutes without writing any bytes to the SSE stream, so the client gives up
// at 60s and the tool times out. Pushing a progress notification every 15s
// rolls the stream and keeps the connection alive. See
