...
func TestStartProgressHeartbeat_NoToken(t *testing.T) {
	...
	stop := startProgressHeartbeat(context.Background(), fake, nil, "convert_markdown", 10*time.Millisecond)
	if stop == nil {
...
func TestStartProgressHeartbeat_NilSession(t *testing.T) {
	...
	stop := startProgressHeartbeat(context.Background(), nil, "tok-1", "convert_markdown", 10*time.Millisecond)
	stop()
...
func TestStartProgressHeartbeat_NonPositiveInterval(t *testing.T) {
	fake := &fakeNotifier{}
	stop := startProgressHeartbeat(context.Background(), fake, "tok-1", "convert_markdown", 0)
	stop()
	if got := fake.calls.Load(); got != 0 {
...
func TestStartProgressHeartbeat_FiresOnTick(t *testing.T) {
	...
	before := runtime.NumGoroutine()
	stop := startProgressHeartbeat(context.Background(), fake, "tok-1", "convert_markdown", interval)
...
func TestStartProgressHeartbeat_StopsOnContextCancel(t *testing.T) {
	...
	before := runtime.NumGoroutine()
	stop := startProgressHeartbeat(ctx, fake, "tok-1", "convert_markdown", 20*time.Millisecond)
...
func TestStartProgressHeartbeat_StopIsIdempotent(t *testing.T) {
	fake := &fakeNotifier{}
	stop := startProgressHeartbeat(context.Background(), fake, "tok-1", "convert_markdown", time.Second)
