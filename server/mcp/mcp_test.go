...
func TestMCPHandlerExecutionTokenEnforcesToolCallScope(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{name: "progress plan rejects repeated current identity", toolName: "set_task_progress_plan", arguments: `{"task_id":"task-1","execution_id":"execution-1","stages":[]}`},
		{name: "progress update rejects repeated current identity", toolName: "update_task_progress", arguments: `{"task_id":"task-1","execution_id":"execution-1","stage":"one","state":"active"}`},
		{name: "progress rejects null legacy identity", toolName: "update_task_progress", arguments: `{"task_id":"task-1","execution_id":null,"stage":"one","state":"active"}`},
		{name: "metadata submission rejects null legacy identity", toolName: "submit_completion_metadata", arguments: `{"task_id":"task-1","execution_id":null,"metadata":"{}"}`},
