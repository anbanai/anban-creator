...
func TestBillingRepositoryChargeAllocationsSupportExactReversal(t *testing.T) {
	...
	promotionalLot := &model.BillingCreditLot{
		...
		OriginalCredits: 40, ConsumedCredits: 40, ExpiresAt: ptrBillingTime(time.Now().Add(time.Hour)),
	}
...
func TestBillingRepositorySettlementOutboxClaimRetryAndProcessedState(t *testing.T) {
	...
	due2 := billingSettlement("settlement-2", "key-2", "retry", ptrBillingTime(now.Add(-time.Minute)), now.Add(-time.Minute))
	due2.LastError = "previous transient error"
	future := billingSettlement("settlement-future", "key-future", "retry", ptrBillingTime(now.Add(time.Minute)), now)
...
func ptrBillingTime(value time.Time) *time.Time { return &value }
