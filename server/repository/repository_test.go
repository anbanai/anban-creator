...
func TestUserConfigRepository_Upsert(t *testing.T) {
	db := setupTestDB(t)
	repo := New(db)
	ctx := context.Background()
	...
	cfg := &model.UserConfig{
		...
		WechatAppID:  "wx123",
		WechatSecret: "secret123",
		Name:         "My Blog",
	}

	// Upsert (insert)
	if err := repo.UserConfigs().Upsert(ctx, cfg); err != nil {
		t.Fatalf("Upsert insert: %v", err)
	}

	found, err := repo.UserConfigs().FindByUserAndScope(ctx, "user-cfg-1", model.ScopeArticle)
	if err != nil {
		t.Fatalf("FindByUserAndScope: %v", err)
	}
	if found.WechatAppID != "wx123" {
		t.Errorf("expected app_id wx123, got %s", found.WechatAppID)
	}

	// Upsert (update)
	cfg.WechatAppID = "wx456"
	cfg.Name = "Updated Blog"
	if err := repo.UserConfigs().Upsert(ctx, cfg); err != nil {
		t.Fatalf("Upsert update: %v", err)
	}
	found, _ = repo.UserConfigs().FindByUserAndScope(ctx, "user-cfg-1", model.ScopeArticle)
	if found.WechatAppID != "wx456" {
		t.Errorf("expected app_id wx456 after upsert, got %s", found.WechatAppID)
	}

	// ListByUserID
	_ = repo.UserConfigs().Upsert(ctx, &model.UserConfig{
		UserID:      "user-cfg-1",
		Scope:       model.ScopeXls,
		Name:        "My XLS",
	})
	configs, err := repo.UserConfigs().ListByUserID(ctx, "user-cfg-1")
	if err != nil {
	...
	// Delete
	if err := repo.UserConfigs().Delete(ctx, found.ID); err != nil {
		t.Fatalf("Delete: %v", err)
	}
	configs, _ = repo.UserConfigs().ListByUserID(ctx, "user-cfg-1")
	if len(configs) != 1 {
		t.Errorf("expected 1 config after delete, got %d", len(configs))
	}
}
