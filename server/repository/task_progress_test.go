...
func createStructuredProgressTask(t *testing.T, repo Repository) *model.Task {
	t.Helper()
	task := &model.Task{ID: uuid.NewString(), UserID: uuid.NewString(), Type: model.PlatformMoments, Prompt: "progress"}
...
func TestTaskRepositoryAdvanceStructuredProgressRejectsLowerAndDuplicateEvents(t *testing.T) {
	...
	advanced, persistedPayload, err := repo.Tasks().AdvanceStructuredProgress(ctx, task.ID, 10, high)
	if err != nil || !advanced {
	...
	for _, item := range []struct {
		...
		advanced, _, err = repo.Tasks().AdvanceStructuredProgress(ctx, task.ID, item.sequence, item.payload)
		if err != nil || advanced {
...
func TestTaskRepositoryAdvanceStructuredProgressIsAtomicHighWater(t *testing.T) {
	...
	for _, item := range items {
		...
		go func() {
			defer wg.Done()
			<-start
			_, _, err := repo.Tasks().AdvanceStructuredProgress(context.Background(), task.ID, item.sequence, item.payload)
			errs <- err
		}()
	}
	close(start)
...
func TestTaskRepositoryAdvanceStructuredProgressPersistsZeroPercent(t *testing.T) {
	...
	advanced, persistedPayload, err := repo.Tasks().AdvanceStructuredProgress(context.Background(), task.ID, 1, payload)
...
func TestTaskRepositoryAdvanceStructuredProgressRetainsMaximumPercentInLaterPayload(t *testing.T) {
	...
	if advanced, _, err := repo.Tasks().AdvanceStructuredProgress(ctx, task.ID, 1, first); err != nil || !advanced {
		t.Fatalf("advance first = %v, %v", advanced, err)
	}
	later := model.ProgressPayload{Stage: "second", State: "active", Title: "Second", Percent: 55}
	advanced, persistedPayload, err := repo.Tasks().AdvanceStructuredProgress(ctx, task.ID, 2, later)
	if err != nil || !advanced {
