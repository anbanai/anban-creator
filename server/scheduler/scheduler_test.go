...
func TestTaskProcessorHandlesChannelsCapture(t *testing.T) {
	...
	if err := processor.mux.ProcessTask(context.Background(), asynq.NewTask(TypeChannelsCaptureMetrics, []byte(`{"tracking_id":"channels-1"}`))); err != nil {
		t.Fatal(err)
	}
