...
func (h *AIEntryHandler) Submit(c fiber.Ctx) error {
	...
	urls := make([]string, 0, len(req.Attachments))
	for i := range req.Attachments {
		req.Attachments[i] = normalizeHandlerEntryAttachment(req.Attachments[i])
		if err := validateHandlerEntryAttachment(&req.Attachments[i]); err != nil {
	...
	if h.pending != nil {
		if err := finalizePendingURLs(c.Context(), h.pending, userID, service.DirectUploadPurposeAIEntryAttachment, urls); err != nil {
			return Error(c, fiber.StatusBadRequest, err.Error())
		}
	}
	if h.submitter == nil {
