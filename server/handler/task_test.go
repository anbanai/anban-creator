...
func TestTaskHandlerMarkPublishedDoesNotBindSeednoteIdentity(t *testing.T) {
	db := setupTaskHandlerTestDB(t)
	repo := repository.New(db)
	ctx := context.Background()
	...
	req := httptest.NewRequest(http.MethodPatch, "/tasks/"+taskID+"/published", strings.NewReader(`{"published":true,"note_id":"invalid/id","note_url":"https://example.com/explore/note-1"}`))
	req.Header.Set("Content-Type", "application/json")
	resp, err := app.Test(req)
	if err != nil {
		t.Fatal(err)
	...
	if !found.Published {
		t.Fatal("published status must be independent from analytics binding")
	}
}
