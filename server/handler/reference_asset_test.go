...
func TestRejectLegacyReferenceImageURL(t *testing.T) {
	tests := []struct {
		...
		{name: "removed provider strategy field", body: `{"provider_strategy_override":"quality"}`, wantErr: true, wantProviderStrategy: true},
		{name: "removed nested provider strategy field", body: `{"ecommerce":{"PROVIDER_STRATEGY_OVERRIDE":"quality"}}`, wantErr: true, wantProviderStrategy: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantProviderStrategy && !strings.Contains(err.Error(), "use image_capability_key") {
				t.Fatalf("removed provider strategy error = %q, want capability migration hint", err)
			}
		})
	}
}
...
func TestProviderStrategyOverrideIsRejectedByPublicWriteAPIs(t *testing.T) {
	logger := zerolog.New(io.Discard)
	taskHandler := NewTaskHandler(nil, &logger)
	projectHandler := NewProjectHandler(nil, &logger)
	...
	tests := []struct {
		...
		{name: "task", path: "/tasks", body: `{"provider_strategy_override":"quality"}`, run: taskHandler.Create},
		{name: "nested project default", path: "/projects", body: `{"ecommerce_defaults":{"provider_strategy_override":"quality"}}`, run: projectHandler.Create},
	}
