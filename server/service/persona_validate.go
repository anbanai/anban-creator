...
// ErrBylineIsWriterName is returned when a publish byline (作者署名) is set to a
// known 写作风格 writer persona name or key. The byline is the real publishing
...
var ErrBylineIsWriterName = fmt.Errorf("byline must not be a writer persona name")

// RejectWriterNameAsByline returns ErrBylineIsWriterName (wrapping a descriptive
// message) when the supplied byline is a known 写作风格 writer persona name or
// resource key. An empty byline is allowed (means "no explicit byline; resolve
// from the precedence chain").
//
// Defense in depth: the Studio PersonaBlock no longer writes the writer name
// into the byline field, and this guard rejects any other path (direct API,
...
func RejectWriterNameAsByline(author string) error {
	...
	for _, w := range resources.Manager().List(resources.CategoryWriter) {
		...
		if strings.ToLower(display) == lower || strings.ToLower(key) == lower {
			...
			return fmt.Errorf(
				...
				ErrBylineIsWriterName, a, canonical, key,
			)
		}
	}
	return nil
