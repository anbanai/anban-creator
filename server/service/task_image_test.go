...
func TestGenerateTaskImageMaterializesProjectPortraitInput(t *testing.T) {
	for _, tt := range []struct {
		...
		{name: "selected portrait", use: true},
		{name: "portrait and product", use: true, product: true},
		{name: "available but not selected"},
		{name: "foreign portrait", use: true, foreign: true},
		{name: "wrong asset purpose", use: true, wrongPurpose: true},
		{name: "missing frozen portrait", use: true, missing: true},
	} {
		t.Run(tt.name, func(t *testing.T) {
			f := newTaskImageFixture(t)
			ctx := t.Context()
			...
			purpose := DirectUploadPurposeProjectPortraitReference
			if tt.wrongPurpose {
				purpose = DirectUploadPurposeProjectReference
			}
			if err := f.repo.Assets().Create(ctx, &model.Asset{ID: assetID, UserID: owner, Purpose: purpose, StorageKey: key, FileName: "portrait.png", ContentType: "image/png", Size: int64(len(imageBytes)), ETag: "immutable"}); err != nil {
				t.Fatal(err)
			}
			task, err := f.repo.Tasks().FindByID(ctx, f.taskID)
			if err != nil {
			...
			task.SetProjectSnapshot(model.ProjectSnapshot{Platform: model.PlatformArticle, PortraitReferenceImageAssetID: portraitID})
			if err := f.repo.Tasks().Update(ctx, task); err != nil {
				t.Fatal(err)
			}
			var generated bytes.Buffer
			if err := png.Encode(&generated, image.NewRGBA(image.Rect(0, 0, 4, 3))); err != nil {
				t.Fatal(err)
			}
			if err := os.WriteFile(f.generator.result.LocalFilePath, generated.Bytes(), 0600); err != nil {
				t.Fatal(err)
			}
			if tt.product {
				productID := uuid.NewString()
				productKey := "assets/users/" + f.userID + "/" + productID + "/product.png"
				if _, err := f.service.tasks.Storage().Upload(ctx, productKey, bytes.NewReader(generated.Bytes()), "image/png"); err != nil {
					t.Fatal(err)
				}
				if err := f.repo.Assets().Create(ctx, &model.Asset{ID: productID, UserID: f.userID, Purpose: DirectUploadPurposeTaskReference, StorageKey: productKey, FileName: "product.png", ContentType: "image/png", Size: int64(generated.Len()), ETag: "immutable"}); err != nil {
					t.Fatal(err)
				}
				task.ReferenceImageAssetID = productID
				if err := f.repo.Tasks().Update(ctx, task); err != nil {
					t.Fatal(err)
				}
			}
			...
			req.AspectRatio = "4:3"
			if tt.use {
				req.ReferencePaths = []string{".anban-creator/project-portrait-reference.png"}
			}
			if tt.product {
				req.ReferencePaths = append(req.ReferencePaths, ".anban-creator/task-reference.png")
			}
			_, err = f.service.Generate(ctx, req)
			if tt.foreign || tt.wrongPurpose || tt.missing {
				if err == nil || f.generator.calls != 0 {
					t.Fatalf("unauthorized portrait reached generation: err=%v calls=%d", err, f.generator.calls)
				}
				return
			}
			if err != nil {
			...
			if tt.use {
				...
				if len(f.generator.referenceData) != wantCount || !bytes.Equal(f.generator.referenceData[0], imageBytes) {
					t.Fatalf("portrait bytes = %#v", f.generator.referenceData)
				}
				if tt.product && !bytes.Equal(f.generator.referenceData[1], generated.Bytes()) {
					t.Fatal("product reference replaced by portrait")
				}
			} else if len(f.generator.referenceData) != 0 {
				t.Fatal("optional portrait injected without Agent selection")
			}
		})
	}
}
