...
func TestUploadExecutionTaskFileWithSettlementPersistsArtifactAndOutboxAtomically(t *testing.T) {
	...
	store := &fakeTaskArtifactStorage{name: "oss"}
	svc := NewTaskService(fixture.repo, nil, &mockEnqueuer{}, store, nil, &logger, "", nil, "", nil, nil)
	...
	broken := NewTaskService(fixture.repo, nil, &mockEnqueuer{}, store, nil, &logger, "", nil, "", nil, nil)
