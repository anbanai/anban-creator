...
func TestPlanService_Update(t *testing.T) {
	...
	// Update title only.
	updated, err := svc.Update(ctx, created.ID, "", "new hint", nil, "", nil)
	if err != nil {
	...
	updated, err = svc.Update(ctx, created.ID, "0 18 * * *", "new hint", nil, "", nil)
	if err != nil {
	...
	_, err = svc.Update(ctx, created.ID, "bad cron", "hint", nil, "", nil)
	if err == nil {
...
func TestPlanService_Update_SkipReferenceImage(t *testing.T) {
	...
	skipRef := true
	updated, err := svc.Update(ctx, created.ID, "", "new hint", &skipRef, "", nil)
	if err != nil {
	...
	updated, err = svc.Update(ctx, created.ID, "", "unchanged hint", nil, "", nil)
	if err != nil {
	...
	skipRef = false
	updated, err = svc.Update(ctx, created.ID, "", "final hint", &skipRef, "", nil)
	if err != nil {
