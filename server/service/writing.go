...
func NewWritingService(repo repository.Repository, creditSvc *CreditService, llmClient LLMClient, logger *zerolog.Logger) *WritingService {
...
func (s *WritingService) ConvertMarkdown(
	...
	// converter.NewConverter requires a zerolog.Logger; we use a no-op logger
	// since we do our own logging via zerolog.
	cvt := converter.NewConverter(zerolog.Nop())
