...
func TestSettleAgentRuntimeDoesNotChargeOrLockDelivery(t *testing.T) {
	ctx := context.Background()
	svc, fixture := newRuntimeBillingCreditService(t, 9000)
	taskID := "task-runtime-platform-paid"
	...
	result := &serveragent.ExecutionResult{
		Success:      true,
		Model:        "claude-test",
		SessionID:    "sess-runtime-platform-paid",
	...
	if user.CreditsBalance != 5000 {
		t.Fatalf("balance = %d, want unchanged after base fee", user.CreditsBalance)
	}
...
func TestSettleAgentRuntimeClearsLegacyPaymentRequiredWithoutCharging(t *testing.T) {
	ctx := context.Background()
	svc, fixture := newRuntimeBillingCreditService(t, 9000)
	taskID := "task-runtime-legacy-shortfall"
	task := seedRuntimeBillingTask(t, fixture.repo, fixture.userID, taskID)
