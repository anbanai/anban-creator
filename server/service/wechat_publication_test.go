...
func newUnavailableFormalPublishFixture(t *testing.T) *publicationFixture {
	t.Helper()
	f := newPublicationFixture(t)
	f.svc.recordCapability(context.Background(), f.projectID, model.WechatCapabilityFreePublishSubmit, model.WechatCapabilityDenied, 48001)
	return f
}
...
func (f *publicationFixture) seedAmbiguousTaskOutcome(t *testing.T, publication *model.WechatPublication) {
	t.Helper()
	evidence, err := json.Marshal(draftDeliveryEvidence{
		Source: "create_draft", Status: model.TaskExecutionDraftDeliveryAmbiguous,
		Code: "create_draft_pending_reconciliation", Attempt: 1, Attempted: true,
		Action: "check_wechat", OccurredAt: f.now.Add(-time.Minute).Format(time.RFC3339),
	})
	if err != nil {
		t.Fatal(err)
	}
	if err := f.db.WithContext(context.Background()).Model(&model.TaskExecution{}).Where("id = ?", f.executionID).Updates(map[string]any{
		"status": model.TaskExecutionSucceeded, "draft_delivery_status": model.TaskExecutionDraftDeliveryAmbiguous,
		"draft_delivery_result": datatypes.JSON(evidence),
	}).Error; err != nil {
		t.Fatal(err)
	}
	outcome := model.TaskOutcome{
		...
		Review:       model.TaskReviewOutcome{Status: model.TaskReviewPassed},
		Publication: model.TaskPublicationOutcome{
			Status: model.TaskPublicationAmbiguous, Code: "create_draft_pending_reconciliation",
			Message: "pending", Attempted: true, Action: "check_wechat",
		},
		Warnings: []model.TaskOutcomeWarning{{Code: "visual_partial", Stage: "visual", Message: "keep"}},
	}
	if err := f.db.WithContext(context.Background()).Model(&model.Task{}).Where("id = ?", f.taskID).Updates(map[string]any{
		"status": model.TaskStatusCompleted, "outcome": datatypes.NewJSONType(outcome),
	}).Error; err != nil {
		t.Fatal(err)
	}
	if publication != nil {
		publication.TaskID, publication.ExecutionID = f.taskID, f.executionID
		publication.UserID, publication.ProjectID = f.userID, f.projectID
	}
}

func TestScheduledDraftReconciliationSynchronizesTaskOutcomeWithoutResubmission(t *testing.T) {
	f := newPublicationFixture(t)
	request := f.draftInput()
	article := normalizeDraftArticle(request.Articles[0])
	attemptedAt := f.now.Add(-time.Minute)
	publication := &model.WechatPublication{
		ID: uuid.NewString(), Source: model.WechatPublicationSourceAnbanAPI,
		Status: model.WechatPublicationStatusDrafting, DraftAddAttemptedAt: &attemptedAt, DraftCreatedAt: &attemptedAt,
		...
		DraftRequestFingerprint: wechatDraftRequestFingerprint(article), NextCheckAt: &f.now,
	}
	f.seedAmbiguousTaskOutcome(t, publication)
	...
	f.api.draftListResponse = &appwechat.DraftBatchGetResponse{TotalCount: 1, ItemCount: 1, Items: []appwechat.DraftBatchItem{{
		MediaID: "reconciled-draft", UpdateTime: f.now.Unix(), Content: appwechat.DraftContent{NewsItems: []appwechat.DraftArticle{article}},
	}}}
	...
	if f.api.addCalls != 0 {
		t.Fatalf("draft/add calls = %d, want 0", f.api.addCalls)
	}
	execution, err := f.repo.TaskExecutions().FindByID(context.Background(), f.executionID)
	if err != nil {
		t.Fatal(err)
	}
	if execution.DraftDeliveryStatus != model.TaskExecutionDraftDeliverySucceeded {
		t.Fatalf("execution delivery status = %q, want succeeded", execution.DraftDeliveryStatus)
	}
	task, err := f.repo.Tasks().FindByID(context.Background(), f.taskID)
	if err != nil {
		t.Fatal(err)
	}
	if task.Outcome == nil || task.Outcome.Publication.Status != model.TaskPublicationSucceeded || !task.Outcome.Publication.Attempted || task.Outcome.Publication.Action != "" {
		t.Fatalf("task publication outcome = %#v, want attempted success", task.Outcome)
	}
	if task.Outcome.Review.Status != model.TaskReviewPassed || task.Outcome.Visual.Status != model.TaskVisualPartial || len(task.Outcome.Warnings) != 1 || task.Outcome.Warnings[0].Code != "visual_partial" {
		t.Fatalf("non-publication outcome fields changed: %#v", task.Outcome)
	}
}

func TestReconciledDraftDeliverySynchronizationHandlesPendingAndDefinitiveFailure(t *testing.T) {
	tests := []struct {
		name              string
		...
		{name: "still pending", publicationStatus: model.WechatPublicationStatusDrafting, wantStatus: model.TaskExecutionDraftDeliveryAmbiguous, wantCode: "create_draft_pending_reconciliation", wantAction: "check_wechat"},
		{name: "definitive unsupported", publicationStatus: model.WechatPublicationStatusUnsupported, wantStatus: model.TaskExecutionDraftDeliveryFailed, wantCode: "create_draft_unsupported", wantAction: "retry_draft", retryAuthorized: true},
		{name: "reconciliation permission failure", publicationStatus: model.WechatPublicationStatusUnsupported, wantStatus: model.TaskExecutionDraftDeliveryFailed, wantCode: "create_draft_reconciliation_failed", wantAction: "check_wechat"},
		{name: "expired unknown outcome requires manual check", publicationStatus: model.WechatPublicationStatusPublishFailed, wantStatus: model.TaskExecutionDraftDeliveryFailed, wantCode: "create_draft_reconciliation_failed", wantAction: "check_wechat"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			f := newPublicationFixture(t)
			attemptedAt := f.now.Add(-time.Minute)
			publication := &model.WechatPublication{ID: uuid.NewString(), Source: model.WechatPublicationSourceAnbanAPI, Status: tt.publicationStatus, DraftAddAttemptedAt: &attemptedAt, DraftAddAttempts: 1}
			if tt.publicationStatus == model.WechatPublicationStatusUnsupported {
				publication.WechatStatusCode = 48001
			}
			if tt.retryAuthorized {
				publication.DraftRetryAuthorizedAt = &attemptedAt
			}
			f.seedAmbiguousTaskOutcome(t, publication)

			if err := f.svc.syncReconciledDraftDelivery(context.Background(), publication); err != nil {
				t.Fatalf("sync reconciled delivery: %v", err)
			}
			execution, err := f.repo.TaskExecutions().FindByID(context.Background(), f.executionID)
			if err != nil {
				t.Fatal(err)
			}
			if execution.DraftDeliveryStatus != tt.wantStatus {
				t.Fatalf("execution status = %q, want %q", execution.DraftDeliveryStatus, tt.wantStatus)
			}
			task, err := f.repo.Tasks().FindByID(context.Background(), f.taskID)
			if err != nil {
				t.Fatal(err)
			}
			if task.Outcome == nil || task.Outcome.Publication.Code != tt.wantCode || task.Outcome.Publication.Action != tt.wantAction || !task.Outcome.Publication.Attempted {
				t.Fatalf("task publication outcome = %#v", task.Outcome)
			}
			if tt.publicationStatus == model.WechatPublicationStatusPublishFailed {
				message := task.Outcome.Publication.Message
				if !strings.Contains(message, "无法确认") || !strings.Contains(message, "不会重复提交") {
					t.Fatalf("expired unknown outcome message = %q, want manual-check and no-resubmit guidance", message)
				}
			}
		})
	}
...
func TestCreateDraftRetriesExactUnsupportedPublicationAfterExecutionHandoffToManualPublish(t *testing.T) {
	f := newUnavailableFormalPublishFixture(t)
	ctx := context.Background()
	if err := f.db.Model(&model.TaskExecution{}).Where("id = ?", f.executionID).Update("status", model.TaskExecutionFailed).Error; err != nil {
		t.Fatal(err)
	...
	if retried.ExecutionID != newExecutionID || retried.Status != model.WechatPublicationStatusAwaitingManual ||
		!retried.ManualPublishRequired || retried.WechatStatusCode != 48001 || retried.DraftMediaID != "draft-after-handoff" {
		t.Fatalf("retried publication = %#v", retried)
	}
	if f.api.addCalls != 1 || f.api.submitCalls != 0 {
		t.Fatalf("provider calls add=%d submit=%d, want 1/0", f.api.addCalls, f.api.submitCalls)
	}
}

func TestCreateDraftWithDeniedFormalPublishCapabilityDoesNotScheduleReconciliation(t *testing.T) {
	f := newUnavailableFormalPublishFixture(t)
	f.api.draftListResponse = &appwechat.DraftBatchGetResponse{}
	f.api.addResponse = &appwechat.DraftAddResponse{MediaID: "draft-1"}
	...
	if publication.Status != model.WechatPublicationStatusAwaitingManual || !publication.ManualPublishRequired ||
		publication.WechatStatusCode != 48001 || publication.NextCheckAt != nil {
		t.Fatalf("manual publication fallback = %#v", publication)
	}
	if len(enqueuer.delayed) != 0 || f.api.submitCalls != 0 {
...
func TestCreateDraftRecoversResponseLossBeforeRetrying(t *testing.T) {
	...
	if publication.DraftMediaID != "recovered-media" || publication.Status != model.WechatPublicationStatusAwaitingManual ||
		!publication.ManualPublishRequired || publication.WechatStatusCode != 48001 {
...
func TestCreateDraftPersistsIntentBeforeExternalCallAndDoesNotBlindRetry(t *testing.T) {
	...
	f.api.draftListResponse = &appwechat.DraftBatchGetResponse{}
	_, err = f.svc.CreateDraft(context.Background(), f.userID, f.taskID, f.projectID, f.executionID, f.draftInput())
	if !errors.Is(err, ErrWechatPublicationPending) || f.api.draftListCalls != 1 || f.api.addCalls != 1 {
		t.Fatalf("retry err=%v calls=%d/%d; want pending and no second add", err, f.api.draftListCalls, f.api.addCalls)
	}
}

func TestCreateDraftRecordsProviderSubmissionFailureAsAmbiguous(t *testing.T) {
	f := newPublicationFixture(t)
	f.api.draftListResponse = &appwechat.DraftBatchGetResponse{}
	f.api.addError = errors.New("connection reset after WeChat may have accepted draft")

	if _, err := f.svc.CreateDraft(context.Background(), f.userID, f.taskID, f.projectID, f.executionID, f.draftInput()); !errors.Is(err, ErrWechatPublicationPending) {
		t.Fatalf("first CreateDraft error = %v, want pending reconciliation", err)
	}
	execution, err := f.repo.TaskExecutions().FindByID(context.Background(), f.executionID)
	if err != nil {
		t.Fatal(err)
	}
	if execution.DraftDeliveryStatus != model.TaskExecutionDraftDeliveryAmbiguous {
		t.Fatalf("draft delivery status = %q, want ambiguous", execution.DraftDeliveryStatus)
	}

	f.api.addError = nil
	f.api.addResponse = &appwechat.DraftAddResponse{MediaID: "unexpected-duplicate-draft"}
	if _, err := f.svc.CreateDraft(context.Background(), f.userID, f.taskID, f.projectID, f.executionID, f.draftInput()); !errors.Is(err, ErrWechatPublicationPending) {
		t.Fatalf("second CreateDraft error = %v, want pending reconciliation", err)
	}
	if f.api.addCalls != 1 {
...
func TestCreateDraftRejectsMissingExecutionBeforeProviderUse(t *testing.T) {
	f := newPublicationFixture(t)
	if err := f.db.Delete(&model.TaskExecution{}, "id = ?", f.executionID).Error; err != nil {
		t.Fatal(err)
	}
	f.api.draftListResponse = &appwechat.DraftBatchGetResponse{}
	f.api.addResponse = &appwechat.DraftAddResponse{MediaID: "unexpected-draft"}

	if _, err := f.svc.CreateDraft(context.Background(), f.userID, f.taskID, f.projectID, f.executionID, f.draftInput()); !errors.Is(err, ErrWechatPublicationExecutionMismatch) {
		t.Fatalf("CreateDraft error = %v, want execution mismatch", err)
	}
	if f.api.draftListCalls != 0 || f.api.addCalls != 0 {
		t.Fatalf("missing execution reached WeChat: list=%d add=%d", f.api.draftListCalls, f.api.addCalls)
	}
}

func TestCreateDraftRecordsPreflightFailureOnCurrentExecution(t *testing.T) {
	f := newPublicationFixture(t)
	request := f.draftInput()
	request.Articles[0].Title = "添加我微信领取茶叶资料"

	if _, err := f.svc.CreateDraft(context.Background(), f.userID, f.taskID, f.projectID, f.executionID, request); !errors.Is(err, ErrWechatPublicationMarketingBlocked) {
		t.Fatalf("CreateDraft error = %v, want marketing block", err)
	}
	execution, err := f.repo.TaskExecutions().FindByID(context.Background(), f.executionID)
	if err != nil {
		t.Fatal(err)
	}
	if execution.DraftDeliveryStatus != "" || len(execution.DraftDeliveryResult) != 0 {
		t.Fatalf("preflight failure recorded a provider attempt: status=%q evidence=%s", execution.DraftDeliveryStatus, execution.DraftDeliveryResult)
	}
	if f.api.draftListCalls != 0 || f.api.addCalls != 0 {
		t.Fatalf("preflight failure reached WeChat: list=%d add=%d", f.api.draftListCalls, f.api.addCalls)
	}
}

func TestCreateDraftRetryableProviderFailureCanRetryOnce(t *testing.T) {
	f := newPublicationFixture(t)
	f.api.draftListResponse = &appwechat.DraftBatchGetResponse{}
	f.api.addResponse = &appwechat.DraftAddResponse{MediaID: "draft-retry"}
	...
	request := f.draftInput()
	if _, err := f.svc.CreateDraft(context.Background(), f.userID, f.taskID, f.projectID, f.executionID, request); err == nil {
		t.Fatal("first CreateDraft unexpectedly succeeded")
	}
	publication, err := f.svc.CreateDraft(context.Background(), f.userID, f.taskID, f.projectID, f.executionID, request)
	if err != nil {
		t.Fatalf("retry CreateDraft error = %v", err)
	}
	if publication == nil || publication.DraftMediaID == "" {
		t.Fatalf("retry publication = %#v, want created draft", publication)
	}
	execution, err := f.repo.TaskExecutions().FindByID(context.Background(), f.executionID)
	if err != nil {
		t.Fatal(err)
	...
	if execution.DraftDeliveryStatus != model.TaskExecutionDraftDeliverySucceeded {
		t.Fatalf("draft delivery status = %q, want succeeded", execution.DraftDeliveryStatus)
	}
}

func TestCreateDraftRetryableProviderFailureRejectsThirdAttempt(t *testing.T) {
	f := newPublicationFixture(t)
	var factoryCalls int
	f.svc.apiFactory = func(*model.Project) (WechatPublicationAPI, error) {
		factoryCalls++
	...
	for attempt := 1; attempt <= 2; attempt++ {
		if _, err := f.svc.CreateDraft(context.Background(), f.userID, f.taskID, f.projectID, f.executionID, request); err == nil {
			t.Fatalf("CreateDraft attempt %d unexpectedly succeeded", attempt)
		}
	}
	if _, err := f.svc.CreateDraft(context.Background(), f.userID, f.taskID, f.projectID, f.executionID, request); !errors.Is(err, ErrWechatPublicationConflict) {
		t.Fatalf("third CreateDraft error = %v, want conflict", err)
	}
	if factoryCalls != 2 {
		t.Fatalf("API factory calls = %d, want 2", factoryCalls)
	}
}

func TestCreateDraftDoesNotContinueWhileDeliveryIsInFlight(t *testing.T) {
	f := newPublicationFixture(t)
	evidence := []byte(`{"source":"create_draft","status":"in_flight","attempt":1}`)
	if won, err := f.repo.TaskExecutions().TransitionDraftDelivery(context.Background(), f.executionID, "", model.TaskExecutionDraftDeliveryInFlight, evidence); err != nil || !won {
		t.Fatalf("record in-flight delivery: won=%v err=%v", won, err)
	}
	if _, err := f.svc.CreateDraft(context.Background(), f.userID, f.taskID, f.projectID, f.executionID, f.draftInput()); !errors.Is(err, ErrWechatPublicationPending) {
		t.Fatalf("CreateDraft error = %v, want pending", err)
	}
	if f.api.draftListCalls != 0 || f.api.addCalls != 0 {
		t.Fatalf("in-flight duplicate reached WeChat: list=%d add=%d", f.api.draftListCalls, f.api.addCalls)
	}
}
...
func TestCreateDraftReclaimsStaleUnattemptedIntentAfterCrash(t *testing.T) {
	...
	if recovered.ID != intent.ID || recovered.Status != model.WechatPublicationStatusAwaitingManual ||
		!recovered.ManualPublishRequired || recovered.WechatStatusCode != 48001 || recovered.DraftMediaID != "draft-after-crash" {
...
func TestDraftReconciliationPermissionFailurePreservesAmbiguousAddEvidence(t *testing.T) {
	f := newPublicationFixture(t)
	f.api.draftListResponse = &appwechat.DraftBatchGetResponse{}
	f.api.addError = errors.New("connection reset after WeChat may have accepted draft")
	...
	if err != nil || preserved.Status != model.WechatPublicationStatusDrafting || preserved.WechatStatusCode != 48001 ||
		preserved.DraftAddAttemptedAt == nil || preserved.DraftRetryAuthorizedAt != nil {
		t.Fatalf("ambiguous permission failure = %#v err=%v", preserved, err)
	}
	...
	if !errors.Is(err, ErrWechatPublicationPending) || pending == nil || pending.DraftAddAttemptedAt == nil ||
		pending.DraftCreatedAt == nil || !pending.DraftCreatedAt.Equal(originalDraftCreatedAt) || f.api.addCalls != 1 {
		t.Fatalf("same-execution retry = %#v err=%v addCalls=%d; want pending gate without another AddDraft", pending, err, f.api.addCalls)
	}
}
...
func TestCreateDraftRetriesDefinitive48001AfterPermissionRepair(t *testing.T) {
	...
	retried, err := f.svc.CreateDraft(context.Background(), f.userID, f.taskID, f.projectID, newExecutionID, f.draftInput())
	if err != nil {
		t.Fatalf("retried CreateDraft: %v", err)
	}
	if retried.Status != model.WechatPublicationStatusAwaitingManual || !retried.ManualPublishRequired ||
		retried.WechatStatusCode != 48001 || retried.DraftMediaID != "draft-after-repair" || f.api.addCalls != 1 || f.api.submitCalls != 0 {
...
func TestCreateDraftRetriesDefinitive40164AfterIPAllowlistRepair(t *testing.T) {
	...
	if first.Status != model.WechatPublicationStatusUnsupported || first.WechatStatusCode != 40164 || first.DraftAddAttemptedAt == nil || first.DraftAddAttempts != 1 {
		t.Fatalf("definitively rejected draft = %#v", first)
	}
	firstExecution, err := f.repo.TaskExecutions().FindByID(context.Background(), f.executionID)
	if err != nil {
		t.Fatal(err)
	}
	var firstEvidence draftDeliveryEvidence
	if json.Unmarshal(firstExecution.DraftDeliveryResult, &firstEvidence) != nil || !firstEvidence.Attempted || firstEvidence.Action != "retry_draft" {
		t.Fatalf("definitive rejection evidence = %s", firstExecution.DraftDeliveryResult)
	}
	firstAttemptedAt := *first.DraftAddAttemptedAt
	...
	if retried.Status != model.WechatPublicationStatusAwaitingManual || !retried.ManualPublishRequired || retried.WechatStatusCode != 48001 ||
		retried.DraftMediaID != "draft-after-allowlist-repair" ||
		retried.DraftAddAttemptedAt == nil || !retried.DraftAddAttemptedAt.Equal(firstAttemptedAt) || retried.DraftAddAttempts != 2 || f.api.addCalls != 2 {
		t.Fatalf("retried = %#v addCalls=%d", retried, f.api.addCalls)
	}
}

func TestCreateDraftAllowsOnlyOneSafeRetryAfterDefinitiveRejections(t *testing.T) {
	f := newPublicationFixture(t)
	f.api.draftListResponse = &appwechat.DraftBatchGetResponse{}
	f.api.addError = &appwechat.WechatAPIError{ErrCode: 40164, UserMsg: "request ip is not in whitelist"}

	first, err := f.svc.CreateDraft(context.Background(), f.userID, f.taskID, f.projectID, f.executionID, f.draftInput())
	if !errors.Is(err, ErrWechatPublicationDraftRejected) || first.DraftAddAttempts != 1 {
		t.Fatalf("first rejection = %#v err=%v", first, err)
	}

	currentExecutionID := f.executionID
	for attempt := 2; attempt <= 3; attempt++ {
		if err := f.db.Model(&model.TaskExecution{}).Where("id = ?", currentExecutionID).Update("status", model.TaskExecutionFailed).Error; err != nil {
			t.Fatal(err)
		}
		currentExecutionID = uuid.NewString()
		if err := f.db.Model(&model.Task{}).Where("id = ?", f.taskID).Update("current_execution_id", currentExecutionID).Error; err != nil {
			t.Fatal(err)
		}
		if err := f.repo.TaskExecutions().Create(context.Background(), &model.TaskExecution{
			ID: currentExecutionID, TaskID: f.taskID, Attempt: attempt, Target: "test", Status: model.TaskExecutionRunning,
			ExecutionProfile: "effective", Provider: "deepseek", ProfileEnvs: map[string]string{}, ProfileFingerprint: "fingerprint",
		}); err != nil {
			t.Fatal(err)
		}
		publication, retryErr := f.svc.CreateDraft(context.Background(), f.userID, f.taskID, f.projectID, currentExecutionID, f.draftInput())
		if attempt == 2 {
			if !errors.Is(retryErr, ErrWechatPublicationDraftRejected) || publication.DraftAddAttempts != 2 {
				t.Fatalf("safe retry = %#v err=%v", publication, retryErr)
			}
			continue
		}
		if !errors.Is(retryErr, ErrWechatPublicationConflict) || publication.DraftAddAttempts != 2 || f.api.addCalls != 2 {
			t.Fatalf("third submission = %#v err=%v addCalls=%d", publication, retryErr, f.api.addCalls)
		}
	}
}
...
func TestDefinitiveDraftRetryPreflightFailurePreservesOriginalCode(t *testing.T) {
	...
	if err := f.repo.TaskExecutions().Create(context.Background(), &model.TaskExecution{ID: newExecutionID, TaskID: f.taskID, Attempt: 2, Target: "test", Status: model.TaskExecutionRunning, ExecutionProfile: "effective", Provider: "deepseek", ProfileEnvs: map[string]string{}, ProfileFingerprint: "fingerprint"}); err != nil {
		t.Fatal(err)
	}

	f.api.addError = nil
	f.api.draftListError = errors.New("temporary draft-list failure after allowlist repair")
	if _, err := f.svc.CreateDraft(context.Background(), f.userID, f.taskID, f.projectID, newExecutionID, f.draftInput()); err == nil {
	...
	f.api.submitResponse = &appwechat.FreePublishSubmitResponse{PublishID: "publish-after-transient-preflight"}
	f.api.onAdd = func() {
		article := f.draftInput().Articles[0]
		f.api.mu.Lock()
		f.api.draftListResponse = &appwechat.DraftBatchGetResponse{TotalCount: 1, ItemCount: 1, Items: []appwechat.DraftBatchItem{{
			MediaID: "draft-after-transient-preflight", UpdateTime: f.now.Unix(), Content: appwechat.DraftContent{NewsItems: []appwechat.DraftArticle{article}},
		}}}
		f.api.mu.Unlock()
	}
	retried, err := f.svc.CreateDraft(context.Background(), f.userID, f.taskID, f.projectID, newExecutionID, f.draftInput())
	if err != nil || retried.Status != model.WechatPublicationStatusPublishing || retried.DraftMediaID != "draft-after-transient-preflight" ||
		retried.PublishID != "publish-after-transient-preflight" {
...
func TestAmbiguousDraftAddSchedulesExactBatchRecoveryWithoutResubmission(t *testing.T) {
	...
	if err != nil || stored.Status != model.WechatPublicationStatusAwaitingManual || !stored.ManualPublishRequired ||
		stored.WechatStatusCode != 48001 || stored.DraftMediaID != "recovered-draft" {
...
func TestAmbiguousDraftRecoveryReturnsTerminalFailureAfterPermissionRepairAnd72Hours(t *testing.T) {
	...
	replayed, err := f.svc.CreateDraft(context.Background(), f.userID, f.taskID, f.projectID, newExecutionID, f.draftInput())
	if !errors.Is(err, ErrWechatPublicationDraftFailed) || replayed == nil || replayed.Status != model.WechatPublicationStatusPublishFailed {
		t.Fatalf("terminal replay = %#v err=%v, want durable terminal failure", replayed, err)
	}
	if f.api.addCalls != 1 {
...
func TestRetryPublishDoesNotResubmitWhenManualPublishIsRequired(t *testing.T) {
	f := newPublicationFixture(t)
	f.seedDrafted(t)
	f.api.submitError = &appwechat.WechatAPIError{ErrCode: 48001, UserMsg: "api unauthorized"}

	waiting, err := f.svc.Publish(context.Background(), f.userID, f.taskID)
	if err != nil || waiting.Status != model.WechatPublicationStatusAwaitingManual {
		t.Fatalf("first Publish = %#v, %v", waiting, err)
	}
...
func TestPublishUsesDeniedCapabilityWithoutCallingProvider(t *testing.T) {
	f := newPublicationFixture(t)
	f.seedDrafted(t)
	checkedAt := f.now.Add(-time.Hour)
	if err := f.repo.WechatCapabilities().Upsert(context.Background(), &model.WechatAccountCapability{
		ProjectID: f.projectID, Capability: model.WechatCapabilityFreePublishSubmit,
		Status: model.WechatCapabilityDenied, LastCheckedAt: &checkedAt, LastWechatCode: 48001,
	}); err != nil {
		t.Fatal(err)
	}
	...
	if f.api.publishedListCalls != 0 || f.api.draftListCalls != 0 || f.api.submitCalls != 0 {
		t.Fatalf("provider calls: published=%d drafts=%d submit=%d", f.api.publishedListCalls, f.api.draftListCalls, f.api.submitCalls)
	}
}

func TestBindManualPublicationCompletesDraftWithoutCallingProvider(t *testing.T) {
	f := newPublicationFixture(t)
	publication := f.seedDrafted(t)
	...
	bound, err := f.svc.BindManualPublication(context.Background(), f.userID, f.taskID, "https://mp.weixin.qq.com/s?mid=1#rd")
	if err != nil {
		t.Fatal(err)
	}
...
func TestPublishPersists48001FromPreflightAsAwaitingManual(t *testing.T) {
	...
	if publication.Status != model.WechatPublicationStatusAwaitingManual || publication.WechatStatusCode != 48001 || !publication.ManualPublishRequired {
		t.Fatalf("publication = %#v", publication)
	}
	stored, findErr := f.repo.WechatPublications().FindByTaskID(context.Background(), f.taskID)
	if findErr != nil || stored.Status != model.WechatPublicationStatusAwaitingManual {
...
func TestDefinitive48001ClearsSubmissionEvidenceForSafeRetry(t *testing.T) {
	...
	if publication.Status != model.WechatPublicationStatusAwaitingManual || publication.SubmitAttemptedAt != nil {
		t.Fatalf("publication = %#v", publication)
	}
	stored, findErr := f.repo.WechatPublications().FindByTaskID(context.Background(), f.taskID)
...
func TestCachedDeniedFormalPublishCapabilityBypassesBackgroundReconciliation(t *testing.T) {
	f := newUnavailableFormalPublishFixture(t)
	publication := f.seedDrafted(t)
	...
	stored, err := f.repo.WechatPublications().FindByID(context.Background(), publication.ID)
	if err != nil || stored.Status != model.WechatPublicationStatusAwaitingManual || !stored.ManualPublishRequired ||
		stored.WechatStatusCode != 48001 || stored.NextCheckAt != nil {
		t.Fatalf("stored = %#v err=%v", stored, err)
	}
	if f.api.publishedListCalls != 0 {
		t.Fatalf("published-list calls = %d, want 0 for cached denied capability", f.api.publishedListCalls)
	}
}
...
func TestPublishUnsupportedAndMissingDraftNeverBlindSubmit(t *testing.T) {
	t.Run("48001", func(t *testing.T) {
		...
		got, err := f.svc.Publish(context.Background(), f.userID, f.taskID)
		if err != nil || got.Status != model.WechatPublicationStatusAwaitingManual || got.WechatStatusCode != 48001 {
...
func TestReconcileUsesDeniedCapabilityWithoutSubmittingAgain(t *testing.T) {
	f := newPublicationFixture(t)
	f.seedDrafted(t)
	f.svc.recordCapability(context.Background(), f.projectID, model.WechatCapabilityFreePublishSubmit, model.WechatCapabilityDenied, 48001)
	f.api.publishedListResponse = &appwechat.FreePublishBatchGetResponse{}
	...
	got, _ := f.repo.WechatPublications().FindByTaskID(context.Background(), f.taskID)
	if got.Status != model.WechatPublicationStatusAwaitingManual || got.NextCheckAt != nil {
		t.Fatalf("publication after denied capability = %#v", got)
	}
	if f.api.publishedListCalls != 0 || f.api.submitCalls != 0 {
		t.Fatalf("provider calls list=%d submit=%d", f.api.publishedListCalls, f.api.submitCalls)
	}
}

func TestReconcileRateLimitIsAtomicAcrossConcurrentRequests(t *testing.T) {
	f := newPublicationFixture(t)
	publication := f.seedDrafted(t)
	publication.Status = model.WechatPublicationStatusAmbiguous
	publication.SubmitAttemptedAt = &f.now
	if err := f.repo.WechatPublications().Update(context.Background(), publication); err != nil {
		t.Fatal(err)
	}
	f.api.publishedListResponse = &appwechat.FreePublishBatchGetResponse{}
