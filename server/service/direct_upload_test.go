...
func TestFinalizePendingUploadURLsAcceptsEncodedOSSPath(t *testing.T) {
	...
	if _, err := FinalizePendingUploadURLs(context.Background(), store, repo, upload.UserID, upload.Purpose, []string{raw}, now, func(string) bool { return true }); err != nil {
...
func TestFinalizePendingUploadURLsRejectsMalformedPendingKey(t *testing.T) {
	...
	_, err := FinalizePendingUploadURLs(context.Background(), nil, &fakePendingUploadRepo{}, "user-1", DirectUploadPurposeTaskReference, []string{raw}, now, func(string) bool { return true })
...
func TestFinalizePendingUploadURLsIgnoresExternalPendingLikePaths(t *testing.T) {
	...
	for _, raw := range []string{
		...
		rewrites, err := FinalizePendingUploadURLs(
			context.Background(), nil, repo, "user-1", DirectUploadPurposeTaskReference,
		...
		if len(rewrites) != 0 {
			t.Fatalf("FinalizePendingUploadURLs(%q) rewrites = %#v, want none", raw, rewrites)
		}
	}
}

func TestFinalizePendingUploadURLsIgnoresExternalHostBeforeRepoLookup(t *testing.T) {
	now := time.Date(2026, 7, 15, 12, 0, 0, 0, time.UTC)
	upload := &model.PendingUpload{
		ID: "real-upload", UserID: "user-1", Purpose: DirectUploadPurposeTaskReference,
		Key: "uploads/pending/user-1/real-upload/ref.png", PublicURL: "https://cdn.example.com/uploads/pending/user-1/real-upload/ref.png",
		FileName: "ref.png", ContentType: "image/png", Size: 1,
		Status: model.PendingUploadStatusPending, ExpiresAt: now.Add(time.Minute),
	}
	repo := &fakePendingUploadRepo{uploads: map[string]*model.PendingUpload{upload.ID: upload}}
	raw := "https://external.example.com/uploads/pending/user-1/real-upload/ref.png"

	rewrites, err := FinalizePendingUploadURLs(
		context.Background(), matchingDirectUploadStore(upload), repo, upload.UserID, upload.Purpose,
		[]string{raw}, now, func(string) bool { return false },
	)
	if err != nil {
		t.Fatalf("FinalizePendingUploadURLs() error = %v, want external URL ignored", err)
	}
	if len(rewrites) != 0 {
		t.Fatalf("rewrites = %#v, want none", rewrites)
	}
	if len(repo.findCalls) != 0 {
		t.Fatalf("repository lookups = %v, want none", repo.findCalls)
	}
}
