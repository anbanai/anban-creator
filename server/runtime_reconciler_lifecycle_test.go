...
func TestManagedRuntimeReconcilerConfigCoversDockerAndKubernetes(t *testing.T) {
	...
	if docker.ActiveDeadline != 10*time.Minute || docker.HeartbeatTimeout <= 0 || docker.HeartbeatTimeout >= docker.ActiveDeadline || docker.CompletionGrace != 30*time.Second {
	...
	want := serveragent.RuntimeReconcilerConfig{
		ActiveDeadline: 15 * time.Minute, HeartbeatTimeout: 2 * time.Minute, CompletionGrace: 45 * time.Second,
	}
