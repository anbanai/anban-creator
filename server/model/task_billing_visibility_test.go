...
func TestTaskJSONHidesInternalProviderCostEvidence(t *testing.T) {
	...
	encoded := string(raw)
	for _, forbidden := range []string{`"terminal_model_usage"`, `"cost_status"`, `"input_tokens"`, `"output_tokens"`, `"cache_read_tokens"`, `"cache_creation_tokens"`, `"ANTHROPIC_AUTH_TOKEN"`, `"auth_token"`, "secret.invalid", "secret-token"} {
		if strings.Contains(encoded, forbidden) {
			t.Fatalf("task JSON leaked %q: %s", forbidden, encoded)
		}
	}
	for _, required := range []string{"billing_catalog_id", "billing_sku_id", "billing_price_credits", `"agent_profile_snapshot"`, `"provider":"moonshot"`, `"schema_version":3`, `"envs":`, `"ANTHROPIC_BASE_URL":`, `"model_usage_aliases":`} {
