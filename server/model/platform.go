...
type PlatformFieldConfig struct {
	Key         string `json:"key"`          // field name (matches Project struct / request JSON)
...
var PlatformConfigs = map[string]*PlatformConfig{
	PlatformArticle: {
		...
		Fields: []PlatformFieldConfig{
			...
			{Key: "visual_style", Label: "图片视觉", Placeholder: "例如 温暖治愈系水彩、极简几何线条、复古胶片质感", Type: "textarea", Group: "advanced"},
			{Key: "writer_key", Label: "创作风格", Placeholder: "例如 casual-science, dan-koe", Type: "select", Group: "advanced"},
			{Key: "theme", Label: "排版样式", Placeholder: "例如 autumn-warm, spring-fresh", Type: "select", Group: "advanced"},
			{Key: "byline", Label: "作者名", Placeholder: "例如 张三", Type: "text", Group: "advanced"},
