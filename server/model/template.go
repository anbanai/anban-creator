...
//   - 电商 (ecommerce): 图片视觉 (StylePrompt) + Ecommerce defaults (default modules /
//     target platform / brand brief / image model key). No WritingStyle/Theme/作者
//     (those are article/seednote dimensions; ecommerce uses only the Style dimension).
//
...
type Template struct {
	...
	// for type="ecommerce" templates. Zero value for non-ecommerce templates. Merged
	// into Task.Ecommerce at creation (task-level explicit values win); product
	// photos stay per-task. See EcommerceTemplateDefaults.
...
// bakes into every task: which deliverable modules to pre-select (and quantity),
// the default target platform, a brand brief, and the default image model key
// (e.g. "openai-gpt-image"). Stored on Template.Ecommerce as a typed JSON column.
//
// Visual style is NOT here — it flows through StylePrompt (the 图片视觉 dimension),
// like the seednote template. Ecommerce uses only the Style dimension (no
// WritingStyle/Theme/作者). Product photos are intentionally NOT here: each product
// launch ships new photos, so a template can pre-select modules/style/platform/
// model but never the photos themselves.
