...
//     byline — overrides channel.Author via the resolved `author`) + 写作风格
...
//     Surfaced to the agent via get_channel_profile as the resolved top-level
//     `author` (precedence template > channel); the agent passes it to
//     publish_draft. Empty = no override (channel.Author used).
...
//
// WritingStyle (writer 资源 key) is retained for poster and channel resolution;
...
type Template struct {
	...
	// WritingStyle is the writer 资源 key (e.g. "dan-koe") used by poster channels
	// and channel resolution. The article form does NOT set it (it uses the inline
	...
	// and resolved into Task.Theme at creation. Empty = no override (channel theme used).
	Theme string `gorm:"type:varchar(50);default:''" json:"theme"`
	// 作者（署名 byline）— the published WeChat author name. Surfaced to the agent
	// via get_channel_profile as the resolved top-level `author` (precedence
	// template > channel); the agent passes it to publish_draft. JUST a name for
	// 署名 — independent of the 写作风格 fields below. Empty = channel.Author used.
